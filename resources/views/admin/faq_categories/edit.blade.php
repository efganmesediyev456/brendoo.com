...
<div class="main-content">
    <div class="page-content">
        <div class="container-fluid">
            ...
            @endif
            <form action="{{ route('faq_categories.update', $faq_category->id) }}" method="post" enctype="multipart/form-data">
                {{ method_field('PUT') }}
                @csrf
                <div class="card">
                    <div class="card-body">
                        <nav aria-label="breadcrumb" style="margin-bottom: 20px;">
                            <ol class="breadcrumb bg-light p-3 rounded">
                                ...
                                <li class="breadcrumb-item"><a href="{{ route('faq_categories.index') }}">Siyahı</a></li>
                                <li class="breadcrumb-item active" aria-current="page">{{ $faq_category->translate('en')?->title }}</li>
                            </ol>
                        </nav>
                            ...
                            @foreach(['en', 'ru'] as $lang)
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}-tab">
                                    <div class="mb-3">
                                        ...
                                        <input class="form-control" type="text" name="{{ $lang }}_title" value="{{ $faq_category->translate($lang)?->title }}">

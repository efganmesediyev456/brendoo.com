                    ...
                    <div class="card-body">
                        ...
                        <nav aria-label="breadcrumb" style="margin-bottom: 20px;">
                            <ol class="breadcrumb bg-light p-3 rounded">
                                ...
                                <li class="breadcrumb-item"><a href="{{ route('sub_categories.index') }}">Siyahı</a></li>
                                <li class="breadcrumb-item active" aria-current="page">{{ $sub_category->translate('en')?->title }}</li>
                            </ol>
                        </nav>
                            ...
                            @foreach([ 'en', 'ru'] as $lang)
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}-tab">
                                    <div class="mb-3">
                                        ...
                                        <input class="form-control" type="text" name="{{ $lang }}_title" value="{{ $sub_category->translate($lang)?->title }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="category_id" id="" class="form-control">
                                @foreach($categories as $category)
                                    <option value="{{$category->id}}" {{$category->id == $sub_category->category_id ? 'selected' : ''}}>{{$category->title}}</option>
                                @endforeach
                            </select>
                        </div>
